...
def compute_costs(left_descriptor, right_descriptor, descriptor_size, distance, disparity):
    print('\tComputing cost volume...', end='')
    height = left_descriptor.shape[0]
    width = left_descriptor.shape[1]
    x_offset = int(descriptor_size / 2)
    sys.stdout.flush()
    dawn = t.time()
...
def sgm(left, right, descriptor, distance, descriptor_size, num_elements, num_orientations, max_disp, pad):
    """
    ...
    if descriptor == 'hog':
        ...
        left_descriptor, right_descriptor = desc.apply_brief(left, right, descriptor_size, num_elements, pad)
    else:
